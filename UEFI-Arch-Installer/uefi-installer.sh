...
#
#		Made to install archlinux (UEFI VERSION)
############################################
...
disk() {
	...
	printf " \033[1m ${white} \n UEFI or BOIS ? \033[0m \n\n"
	printf " \033[1m ${yellow} [1] UEFI \033[0m \n"
	printf " \033[1m ${yellow} [2] BOIS \033[0m \n"
	printf " \033[1m ${green} Choice: \033[0m"
	read uefiORbios
	echo "uefiORbios=$uefiORbios" >> config.sh
...
SMALLpart() {
    ...
    if [ "$uefiORbios" -eq "1" ]
		...
		printf "\033[1m ${white} Defaulting to BIOS - 1 or 2 not chosen \033[0m"
...
HALFpart() {
		...
		if [ "$uefiORbios" -eq "1" ]
			...
			printf "\033[1m ${green} Defaulting to BIOS - 1 or 2 not chosen \033[0m"
			mkfs.ext4 "$bootpart" -L bootfs
		fi
