...
# Grab var values
# yourdrive, rewtpart, swappart, homepart, bootpart, thechoiceman, encRyesno, encHyesno, encSyesno, uefiORbios
source config.sh
...
uefiinst() {
	...
	#	mkinitcpio -p linux
	#fi
	pacman -Syy efibootmgr dosfstools mtools --noconfirm
...
BOOTload() {
	if [ "$uefiORbios" -eq "1" ]
		then
		printf "\033[1m \n ${white} Preparing Grub-UEFI install \n \033[0m"
		uefiinst
	else
